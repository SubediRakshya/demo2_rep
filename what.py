...
class ReadWriteNode(Node):
    ...
    def __init__(self):
        ...
        self.srv = self.create_service(GetPosition, 'get_position', self.get_position_call>
...
if dxl_comm_result != COMM_SUCCESS:
    ...
    def set_position_callback(self, msg):
        ...
        if dxl_comm_result != COMM_SUCCESS:
            ...
            self.get_logger().error(f'Error: {self.packet_handler.getRxPacketError(dxl_err>
        else:
            self.get_logger().info(f'Set [ID: {msg.id}] [Goal Position: {msg.position}]')

    def get_position_callback(self, request, response):
        dxl_present_position, dxl_comm_result, dxl_error = self.packet_handler.read4ByteTx>
            self.port_handler, request.id, ADDR_PRESENT_POSITION
        )

        if dxl_comm_result != COMM_SUCCESS:
            self.get_logger().error(f'Error: {self.packet_handler.getTxRxResult(dxl_comm_r>
        elif dxl_error != 0:
            self.get_logger().error(f'Error: {self.packet_handler.getRxPacketError(dxl_err>
        else:
